...
<template>
  ...
  <div v-if="$route.params.tipo == 'caja'" class="texto-negro">
    <div v-for="item in globalStore.ProductosEnCaja" :key="item.id">
      <div
        v-if="['Todo', 'Título'].includes(selectedOption)"
      ...
      <div
        v-if="['Todo', 'Ficha Técnica'].includes(selectedOption)"
        class="min-h-screen flex items-center"
      >
        <div class="flex items-stretch w-full">
          <div class="p-3 border-3 rounded-2xl border-gray-500 grow">
            <div class="text-center text-4xl font-semibold underline mb-8">
              CAJÓN #{{ globalStore.CajaSeleccionada?.nombre }}
            </div>
            <p class="text-nowrap text-2xl/8">
              ...
              <b>PESO TOTAL: </b
              >{{ (item.producto.pesoUnitario * item.cantidad).toLocaleString("es-VE") }} KG <br />
              <b>ESTADO: </b
              >{{ item.producto.estadoId ? globalStore.ListasMap[item.producto.estadoId] : ""
              }}<br />
          ...
          <img
            ...
            alt="Foto"
            class="ml-2 rounded-2xl max-w-105 grow"
          />
        </div>
      </div>
    </div>
  ...
  <div v-if="$route.params.tipo == 'solicitud'" class="max-w-xl mx-auto texto-negro">
    ...
    <div class="border-2 rounded-lg mb-4 px-4 py-2 bg-gray-100">
      ...
      <p class="mt-1">
        ...
        {{ globalStore.solicitudActual.fechaCreacion.toLocaleDateString("es-VE") }}
      </p>
    </div>
    ...
    <table class="w-full border-collapse text-sm mb-4">
      ...
      <tbody>
        <template
          v-for="(item, index) in globalStore.solicitudActual.productosCantidad"
          :key="item.productoId"
        >
          <tr class="border text-center">
            <td class="border px-2 py-1 bg-gray-100 font-bold" rowspan="2">{{ index + 1 }}</td>
            <td class="border px-2 py-1">
              {{ productosMap.get(item.productoId)?.nombre ?? item.productoId }}
            </td>
          ...
          <tr>
            <td class="border px-2 py-1" colspan="3">
              ...
              <ul class="list-disc list-inside">
                <li v-for="(ubic, index) in Ubicaciones(item.productoId)" :key="index">
                  {{ ubic }}
                </li>
              </ul>
            </td>
  ...
  <div
    v-else
    class="flex justify-center text-center items-center min-h-screen font-bold texto-negro"
  >
    <div v-if="$route.params.tipo == 'galpon'" class="text-[11rem]">
      <div class="underline">{{ globalStore.GalponSeleccionado!.nombre }}</div>
    ...
    <div v-else-if="$route.params.tipo == 'secciones'" class="w-full grid grid-cols-3 text-[9rem]">
      <div
        v-for="seccion in globalStore.EstanteSeleccionado?.niveles.flatMap((n) => n.secciones)"
        :key="seccion.id"
    ...
    <div v-else-if="$route.params.tipo == 'grupos'" class="text-[8rem]">
      ...
      {{
        globalStore.listaSeleccionada?.nombre.substring(
          globalStore.listaSeleccionada?.nombre.indexOf("-") + 2,
        )
      }}
    </div>
