...
const elementoAMover = ref<{
  id: string;
  nombre: string;
  tipo: "Caja" | "Seccion";
  padreActualId: string;
} | null>(null);
...
const seccionesNivel = computed(() => {
  ...
  return nuevoPadreNivel.value.secciones.filter(
    (s) => s.id !== elementoAMover.value!.padreActualId,
  );
});
...
const productosNombresEnCaja = computed(() => {
  return productosEnCajas.value.reduce(
    (dict, cantidad) => {
...
function encontrarNivelDeSeccion(seccionId: string): Nivel | null {
  return (
    globalStore.EstanteSeleccionado!.niveles.find((n) =>
      n.secciones.some((s) => s.id === seccionId),
    ) ?? null
  );
}
...
function Agregar(tipo: TipoEdicion, nivel?: Nivel, seccion?: Seccion) {
  ...
  itemEdicion.value = {
    ...
    nombre: tipo === "Seccion" ? globalStore.EstanteSeleccionado!.nombre : "",
...
function Editar(
  item: { id: string; nombre: string; ordenDescendente?: boolean },
  tipo: TipoEdicion,
) {
  esNuevo.value = false;
  tipoEdicion.value = tipo;
  ...
  itemEdicion.value = {
    id: item.id,
    nombre: item.nombre,
    ordenDescendente: item.ordenDescendente ?? false,
  };
  dialogVisible.value = true;
}
  ...
  if (esNuevo.value) {
    if (tipoEdicion.value === "Nivel") {
      const nuevoNivel: Nivel = {
        id: itemEdicion.value.id,
        nombre: itemEdicion.value.nombre,
        ordenDescendente: itemEdicion.value.ordenDescendente,
        secciones: [],
      };
    ...
    await RegistrarHistorial(
      ...
      `[${tipoEdicion.value} Creado`,
      null,
      tipoEdicion.value == "Caja"
        ? `Caja ${itemEdicion.value.nombre} (Sección: ${seccionParaNuevoItem.value?.nombre})`
    ...
    if (tipoEdicion.value === "Estante") {
      ...
      if (nivel) {
        nivel.nombre = itemEdicion.value.nombre;
        nivel.ordenDescendente = itemEdicion.value.ordenDescendente;
      }
      ...
      for (const nivel of estante.niveles) {
        ...
        if (seccion) {
          seccion.nombre = itemEdicion.value.nombre;
          break;
        }
      }
      ...
      for (const nivel of estante.niveles) {
        for (const seccion of nivel.secciones) {
          ...
          if (caja) {
            caja.nombre = itemEdicion.value.nombre;
            break;
          }
        }
      }
    ...
    await RegistrarHistorial(
      itemEdicion.value.id,
      `[${tipoEdicion.value}] Modificado`,
      nombreAnteriorEdicion.value,
      itemEdicion.value.nombre,
    );
  }
  ...
  confirm.require({
    ...
    accept: async () => {
      await RegistrarHistorial(
        item.id,
        `[${tipo}] Eliminado`,
        tipo == "Caja" ? `Caja ${item.nombre} (Sección: ${seccion?.nombre})` : item.nombre,
        null,
      );
      const estante = globalStore.EstanteSeleccionado!;
      if (tipo === "Nivel") {
...
async function CajaAnterior() {
  ...
  const seccionesOrdenadas = [...nivel.secciones].sort((a, b) =>
    Ordenar(a, b, nivel.ordenDescendente),
  );
...
async function Mover() {
  ...
  if (elementoAMover.value.tipo === "Caja" && nuevoPadreSeccion.value) {
    ...
    await RegistrarHistorial(
      caja.id,
      "[Caja] Movida",
      `De sección: ${padres.seccion.nombre}`,
      `A sección: ${nuevoPadreSeccion.value.nombre}`,
    );
    ...
    await RegistrarHistorial(
      seccion.id,
      "[Sección] Movida",
      `De nivel: ${nivelActual.nombre}`,
      `A nivel: ${nuevoPadreNivel.value.nombre}`,
    );
  } else {
    return;
  }
...
<template>
  <div id="encabezado" class="flex justify-between md:grid md:grid-cols-3 items-center mb-3">
    ...
    <div class="justify-self-center text-xl">
      ESTANTE {{ globalStore.EstanteSeleccionado!.nombre }}
    </div>
    <div class="justify-self-end">
      <Button
        ...
        @click="router.push('/imprimir/secciones')"
      >
      ...
      <Button
        ...
        variant="outlined"
        class="hidden md:inline-flex mr-2"
        @click="router.push('/imprimir/gruposEstante')"
      >
        <span class="p-button-icon p-button-icon-left pi pi-print" />
      ...
      <Button
        ...
        variant="outlined"
        class="mr-2"
        @click="Editar(globalStore.EstanteSeleccionado!, 'Estante')"
      >
  ...
  <div
    v-else
    v-for="nivel in [...globalStore.EstanteSeleccionado!.niveles].sort((a, b) =>
      Ordenar(a, b, globalStore.EstanteSeleccionado!.ordenDescendente),
    )"
    :key="nivel.id"
  >
      ...
      <template #legend>
        <div class="flex items-center">
          ...
          <Button
            ...
            size="small"
            variant="text"
            @click="Agregar('Seccion', nivel)"
            v-tooltip.bottom="'Agregar Sección'"
            class="ml-2"
          />
          <EditarQuitar
            tamaño="small"
            @editarClick="Editar(nivel, 'Nivel')"
            @quitarClick="Quitar(nivel, 'Nivel')"
            :id-elemento="nivel.id"
            :nombre-elemento="'Nivel ' + nivel.nombre"
          />
        </div>
      </template>
      <div
        class="flex flex-wrap md:flex-nowrap gap-2 justify-center"
        :class="{ 'gap-3': Usuario == null }"
      >
        ...
        <Panel
          v-else
          v-for="seccion in [...nivel.secciones].sort((a, b) =>
            Ordenar(a, b, nivel.ordenDescendente),
          )"
          :key="seccion.id"
          :header="seccion.nombre"
          :pt:header:class="'justify-center'"
          :pt:content:class="'p-0'"
        >
          <template #icons v-if="Usuario">
            <BotonesCompacto
              @agregarClick="Agregar('Caja', undefined, seccion)"
              @moverClick="MostrarDialogoMover(seccion, 'Seccion')"
              @editarClick="Editar(seccion, 'Seccion')"
              @quitarClick="Quitar(seccion, 'Seccion', nivel)"
              :id-elemento="seccion.id"
              :nombre-elemento="'Sección ' + seccion.nombre"
          ...
          <div v-else class="flex">
            ...
            >
              <Button
                ...
                severity="warn"
                :label="
                  (seccion.cajas.length > 1 ? 'C-' : 'Caja ') +
                  caja.nombre +
                  (productosNombresEnCaja[caja.id] == undefined ? '*' : '')
                "
                @click="VerCaja(caja)"
                :pt="{ label: 'text-nowrap', root: 'px-1' }"
                v-tooltip.bottom="{
                  value: productosNombresEnCaja[caja.id] ?? 'Caja vacía',
                  pt: { root: 'min-w-auto max-w-md' },
                }"
              />
              <BotonesCompacto
                v-if="Usuario"
                @moverClick="MostrarDialogoMover(caja, 'Caja')"
                @editarClick="Editar(caja, 'Caja')"
                @quitarClick="Quitar(caja, 'Caja', undefined, seccion)"
                @imprimir-click="ImprimirCaja(caja)"
                :id-elemento="caja.id"
                :nombre-elemento="'Caja ' + caja.nombre"
  ...
  >
    <div class="flex flex-col gap-3 pt-1">
      ...
      <div
        v-if="tipoEdicion === 'Estante' || tipoEdicion === 'Nivel'"
      ...
      <div class="grid grid-cols-3 w-full">
        ...
        <div class="flex items-center justify-self-center">
          <Button
            ...
            @click="CajaAnterior"
            :disabled="deshabilitarCajaAnterior"
          />
          <div class="mx-2 text-xl font-medium">
            Caja {{ globalStore.CajaSeleccionada?.nombre }}
          </div>
          <Button
            ...
            @click="CajaSiguiente"
            :disabled="deshabilitarSiguienteCaja"
          />
        </div>
        <Button
          v-if="Usuario"
          ...
          severity="secondary"
          variant="text"
          v-tooltip.bottom="'Vista Impresion'"
          @click="router.push('/imprimir/caja')"
        />
      </div>
    </template>
    <div v-if="globalStore.ProductosEnCaja.length === 0" class="italic text-muted-color">
      No hay productos en esta caja
    </div>
    <div
      v-else
      v-for="item in globalStore.ProductosEnCaja"
      :key="item.id"
      class="p-2 border-2 rounded-xl border-gray bg-yellow-50 dark:bg-yellow-900 mb-2"
    >
      <div class="flex gap-2">
        <div>
          <img
            ...
            :src="imagenEdicion"
            alt="Foto"
            class="rounded-xl md:w-50 md:h-50"
          />
          <img
            v-else
            :hidden="!item.producto.imagenUrl"
        ...
        <div class="flex-1 flex flex-col gap-1">
          ...
          <div class="flex items-center gap-2">
            <b>Grupo:</b>
            <span v-if="productoEditando != item.producto">{{
              item.producto.grupoId ? globalStore.ListasMap[item.producto.grupoId] : ""
            }}</span>
            <Select
              class="flex-1"
              v-else
              v-model="productoEditando!.grupoId"
              :options="grupos"
              optionValue="id"
              optionLabel="nombre"
            />
          ...
          <div class="flex items-center gap-2">
            <b>Fabricante: </b>
            <span v-if="productoEditando != item.producto">{{
              item.producto.fabricanteId ? globalStore.ListasMap[item.producto.fabricanteId] : ""
            }}</span>
            <Select
              v-else
              v-model="productoEditando!.fabricanteId"
              :options="globalStore.ObtenerLista('fabricantes')"
              optionValue="id"
              optionLabel="nombre"
              class="flex-1"
            />
          ...
          <div class="mt-3">
            ...
            <i
              v-if="productoEditando == item.producto"
              class="pi pi-info-circle ml-2 text-primary"
              v-tooltip.bottom="'La cantida puede modificarse en la página: Movimientos'"
            />
          </div>
          <div class="flex items-center gap-2">
            <b>Peso Unitario:</b>
            <span v-if="productoEditando != item.producto"
              >{{
                item.producto.pesoUnitario.toLocaleString("es-VE", { maximumFractionDigits: 2 })
              }}
              Kg.</span
            >
            <InputNumber
              v-else
              v-model="productoEditando!.pesoUnitario"
              mode="decimal"
              :minFractionDigits="0"
              :maxFractionDigits="2"
              class="flex-1"
          ...
          <div>
            ...
            <span
              >{{
                (item.producto.pesoUnitario * item.cantidad).toLocaleString("es-VE", {
          ...
          <div class="flex items-center gap-2">
            <b>Estado: </b>
            <span v-if="productoEditando != item.producto">{{
              item.producto.estadoId
                ? globalStore.ListasMap[item.producto.estadoId]?.substring(3)
                : ""
            }}</span>
            <Select
              v-else
              v-model="productoEditando!.estadoId"
              :options="estados"
              optionValue="id"
              optionLabel="nombre"
              class="flex-1"
      ...
      <div class="mt-1 pl-2 flex items-center">
        ...
        <div v-if="Usuario" class="flex-none ml-2">
          <Button
            ...
            severity="success"
            variant="outlined"
            @click="EditarProducto(item.producto)"
          />
          <Button
            v-if="productoEditando == item.producto"
          ...
          <Button
            ...
            severity="primary"
            variant="outlined"
            @click="GuardarEdicionProducto(item)"
            v-tooltip.bottom="'Guardar'"
            class="ml-2"
          />
        </div>
      </div>
  ...
  <DialogoEdicion
    id="mover"
    v-model:mostrar="mostrarDialogoMover"
    :encabezado="'Mover ' + elementoAMover?.tipo + ' ' + elementoAMover?.nombre"
    :clickAceptar="Mover"
    :desabilitarAceptar="
      elementoAMover?.tipo === 'Caja' ? nuevoPadreSeccion == null : nuevoPadreNivel == null
    "
  >
    <div class="flex flex-col gap-2">
      <div class="flex gap-2 items-center">
        ...
        <Select
          id="nuevoPadreNivel"
          v-model="nuevoPadreNivel"
          option-label="nombre"
          :options="
            globalStore.EstanteSeleccionado!.niveles.filter(
              (n) => n.id !== elementoAMover?.padreActualId,
            )
          "
        >
        </Select>
      ...
      <div class="flex gap-2 items-center" v-if="elementoAMover?.tipo === 'Caja'">
        ...
        <Select
          id="nuevoPadreSeccion"
          v-model="nuevoPadreSeccion"
          option-label="nombre"
          :options="seccionesNivel"
        />
      </div>
    </div>
  </DialogoEdicion>
