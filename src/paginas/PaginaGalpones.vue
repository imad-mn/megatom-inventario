...
async function Guardar() {
  if (esNuevo.value) {
    ...
    await RegistrarHistorial(
      ...
      "[Área] Creado",
      null,
      `${itemEdicion.value.nombre}-${itemEdicion.value.descripcion}`,
    );
    ...
    if (indice >= 0) {
      ...
      await RegistrarHistorial(
        itemEdicion.value.id,
        "[Área] Modificado",
        `${globalStore.Galpones[indice]?.nombre}-${globalStore.Galpones[indice]?.descripcion}`,
        `${itemEdicion.value.nombre}-${itemEdicion.value.descripcion}`,
      );
...
function Quitar(item: Galpon): void {
  confirm.require({
    header: "Eliminar",
    message: `¿Estás seguro de eliminar: "${item.nombre}" y sus descendientes?`,
...
<template>
  <div class="grid grid-cols-3 items-center mb-10">
    <div class="col-start-2 justify-self-center text-xl">AREAS</div>
    <Button
  ...
  <div v-if="globalStore.Galpones.length === 0" class="italic text-muted-color mt-3">
    No hay Áreas
  </div>
  <div class="flex flex-wrap gap-10 justify-center">
    <div
      v-for="item in globalStore.Galpones.sort((a, b) => a.nombre.localeCompare(b.nombre))"
      :key="item.id"
      class="flex justify-center border rounded-md border-gray-300 bg-gray-100 dark:bg-gray-900 dark:border-gray-700 p-0 md:p-2 w-44"
    >
      <Button
        variant="text"
        @click="Ver(item)"
        v-tooltip.bottom="'Ver Área'"
        class="flex flex-col justify-start"
      >
        <i class="pi pi-warehouse text-6xl mb-4"></i>
        <div class="text-muted-color font-semibold">{{ item.nombre }}</div>
      ...
      <EditarQuitar
        ...
        :boton-imprimir="true"
        @imprimir-click="ImprimirGalpon(item)"
        :vertical="true"
        :id-elemento="item.id"
    ...
    <div class="flex flex-col gap-3 pt-1">
      ...
      <FloatLabel variant="on" class="w-full mt-1">
        ...
        <InputText
          ...
          class="w-full"
          :invalid="!itemEdicion?.descripcion"
          aria-autocomplete="none"
          @keyup.enter="Guardar"
        />
      </FloatLabel>
