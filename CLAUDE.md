...
- **`historialService.ts`** — Helpers to write and query `Historial` entries in Firestore. `RegistrarCambioProducto` performs field-level diffing on a `Producto` and records only changed fields (use this instead of `RegistrarHistorial` when editing products).
...
After any create/update/delete operation, call `RegistrarHistorial(idElemento, accion, anterior?, actual?)` from `historialService.ts`. For product edits, use `RegistrarCambioProducto(id, anterior, posterior)` instead — it diffs fields and only records what changed. The `Stringify()` helper converts a `Producto` to a human-readable string for full before/after snapshots. This is how the `/historial` page is populated.
